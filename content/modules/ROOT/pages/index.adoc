...
. Add the following params to the `vllm_args` sub-section of the `serve` section of the `config.yaml` file with **your api-key**:
+
...
. Change the default host_port from `127.0.0.1:8000` to `0.0.0.0:8000` in the `config.yaml`:
+
...
. Verify the RHEL AI server is using API key authentication by running the following command:
+
